...
- **Frontend**: The SvelteKit + TypeScript application located in the `app/` directory that renders the public portfolio and ***REMOVED*** panel
...
- **Admin_Panel**: The protected section of the Frontend accessible at `/***REMOVED***` routes for managing portfolio content
- **Public_Portfolio**: The public-facing section of the Frontend that displays portfolio content to visitors
- **Database**: The PostgreSQL database storing all portfolio content, ***REMOVED*** credentials, contact messages, and site settings
- **Auth_System**: The JWT-based authentication system using bcrypt password hashing for ***REMOVED*** access
- **Migration_Runner**: The component responsible for executing PostgreSQL schema migrations in order
- **Seed_Runner**: The component responsible for populating the Database with initial portfolio content and a default ***REMOVED*** account
...
2. WHEN migrations are applied, THE Database SHALL contain tables for: identity_console, capability_snapshots, implementation_strengths, professional_dossier, education, work_experiences, projects, certifications, publications, contact_info, contact_messages, site_settings, and ***REMOVED***_users
...
**User Story:** As a developer, I want seed data that populates the database with the current portfolio content and a default ***REMOVED*** account, so that the portfolio remains populated after initial setup.
...
2. WHEN the Seed_Runner executes, THE Database SHALL contain a default ***REMOVED*** account with email `***REMOVED***@portfolio.local` and a bcrypt-hashed password
...
**User Story:** As an ***REMOVED***, I want to log in with email and password to access the ***REMOVED*** panel, so that only authorized users can manage portfolio content.
...
3. WHEN a valid JWT token is included in the Authorization header, THE Auth_System SHALL allow access to protected ***REMOVED*** API endpoints
...
5. WHEN `GET /api/auth/me` is called with a valid token, THE Auth_System SHALL return the authenticated ***REMOVED*** user profile without the password hash
6. THE Auth_System SHALL hash passwords using bcrypt with a minimum cost factor of 10

### Requirement 5: Public API Endpoints

**User Story:** As a visitor, I want the public portfolio to load content from the API, so that the displayed information is always current with what the ***REMOVED*** has published.
...
**User Story:** As an ***REMOVED***, I want full CRUD operations for all portfolio sections through protected API endpoints, so that I can create, read, update, and delete content.

#### Acceptance Criteria

1. THE Backend SHALL expose protected CRUD endpoints (GET list, GET by ID, POST create, PUT update, DELETE) for each content section under the `/api/***REMOVED***/` prefix
...
4. THE Backend SHALL expose `PATCH /api/***REMOVED***/{resource}/{id}/publish` and `PATCH /api/***REMOVED***/{resource}/{id}/unpublish` endpoints to toggle the `is_published` field
5. THE Backend SHALL expose `PUT /api/***REMOVED***/{resource}/reorder` accepting an ordered array of IDs to update `order_number` values for a given section
6. WHEN any ***REMOVED*** endpoint is called without a valid JWT token, THE Backend SHALL return a 401 Unauthorized response
...
**User Story:** As an ***REMOVED***, I want a web-based ***REMOVED*** panel with a dark console-style theme, so that I can manage all portfolio content through an intuitive interface.

#### Acceptance Criteria

1. THE Admin_Panel SHALL be accessible at the `/***REMOVED***` route prefix with sub-routes for login, dashboard, and each content section
2. THE Admin_Panel SHALL use a dark theme with console-style aesthetics including a sidebar navigation menu
3. THE Admin_Panel SHALL display a dashboard at `/***REMOVED***/dashboard` showing summary cards with record counts for each content section
4. WHEN the ***REMOVED*** navigates to a content section, THE Admin_Panel SHALL display records in a table list view with columns for key fields, publish status, and action buttons
5. WHEN the ***REMOVED*** clicks Add or Edit, THE Admin_Panel SHALL display a form editor with appropriate input types and client-side validation for required fields
...
**User Story:** As an ***REMOVED***, I want the ***REMOVED*** panel routes to be protected, so that unauthenticated users cannot access content management features.

#### Acceptance Criteria

1. WHEN an unauthenticated user navigates to any `/***REMOVED***` route except `/***REMOVED***/login`, THE Frontend SHALL redirect the user to `/***REMOVED***/login`
2. WHEN the ***REMOVED*** successfully logs in, THE Frontend SHALL store the JWT token and redirect to `/***REMOVED***/dashboard`
3. WHEN the ***REMOVED*** clicks logout, THE Frontend SHALL clear the stored JWT token and redirect to `/***REMOVED***/login`
4. IF the JWT token expires during an ***REMOVED*** session, THEN THE Frontend SHALL redirect the user to `/***REMOVED***/login` with a session expired notification

### Requirement 10: Contact Messages Management

**User Story:** As an ***REMOVED***, I want to view and manage contact messages submitted by visitors, so that I can read and track incoming communications.
...
2. WHEN the ***REMOVED*** opens a contact message, THE Admin_Panel SHALL mark the message as read by calling the Backend
...
**User Story:** As an ***REMOVED***, I want to manage global site settings, so that I can update the site title, meta description, and other site-wide configurations.

#### Acceptance Criteria

1. THE Admin_Panel SHALL provide a settings form at `/***REMOVED***/settings` for editing site_title, meta_description, favicon_url, logo_url, footer_text, theme_mode, and maintenance_mode
...
**User Story:** As an ***REMOVED***, I want to provide image URLs as strings for all image fields, so that I can reference externally hosted images without file upload complexity.
